...
class SingletonDictMeta(type):
    ...
    def __call__(cls, *args, **kwargs):
        ...
        with cls._lock:
            if cls not in cls._instances:
                instance = super(SingletonDictMeta, cls).__call__(*args, **kwargs)
                cls._instances[cls] = instance
        return cls._instances[cls]


class InheritanceSingletonDict(dict, metaclass=SingletonDictMeta):
    _local_instances = {}
    ...
    def __get_base_class(clazz):
        ...
        bases = clazz.__bases__
        for base in bases:
            if base == InheritanceSingletonDict:
                return clazz
            else:
                base_class = InheritanceSingletonDict.__get_base_class(base)
                if base_class:
                    return base_class
        return None

    def __new__(cls, *args, **kwargs):
        base = InheritanceSingletonDict.__get_base_class(cls)
        if base is None:
            raise ValueError(f"{cls} singleton base not found")

        return super(InheritanceSingletonDict, cls).__new__(cls)
