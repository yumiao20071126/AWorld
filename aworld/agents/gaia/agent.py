...
from aworld.agents.gaia.utils import openai_tool_desc_transform
from aworld.config.conf import AgentConfig
from aworld.core.common import Observation, ActionModel
from aworld.logs.util import logger
from aworld.models.llm import get_llm_model
from aworld.virtual_environments.tools_desc import tool_desc_dict
...
@AgentFactory.register(name="execute", desc="execute agent")
class ExcuteAgent(BaseAgent):
    def __init__(self, conf: AgentConfig, **kwargs):
        ...
        self.tools = openai_tool_desc_transform(tool_desc_dict)
...
"""
    ...
    def policy_action(self,
        ...
        current_round_task = observation.content
        ...
        for traj in self.trajectory:
            input_content += traj[0].info['query']
            if traj[-1].choices[0].message.tool_calls is not None:
                input_content += [
                    {'role': 'assistant', 'content': '', 'tool_calls': traj[-1].choices[0].message.tool_calls}]
            else:
                input_content += [{'role': 'assistant', 'content': traj[-1].choices[0].message.content}]
        input_content += observation.info['query']
        try:
        ...
        return [ActionModel(tool_name='', action_name='', params={'output': content, 'tool_calls': tool_calls})]


@AgentFactory.register(name="plan", desc="plan agent")
