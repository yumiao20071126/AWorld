...
from agents.browser_agents.prompts import AgentMessagePrompt
from agents.common import AgentStepInfo, MessageManagerState, MessageMetadata, LlmResult
from core.common import ToolActionModel, Observation, ActionResult
...
class MessageManager:
    ...
    def add_state_message(
        ...
        if result:
            for r in result:
                if r.keep:
                    if r.content:
                        msg = HumanMessage(content='Action result: ' + str(r.content))
                        self._add_message_with_tokens(msg)
                    if r.error:
                        # if endswith \n, remove it
                        if r.error.endswith('\n'):
    ...
    def _ant_add_message_with_tokens(self, message: BaseMessage, position: int | None = None) -> None:
        """Add message with token count metadata
        position: None for last, -1 for second last, etc.
        """

        token_count = json.loads(message.content)['total_tokens']
        metadata = MessageMetadata(tokens=token_count)
        self.state.history.add_message(message, metadata, position)
