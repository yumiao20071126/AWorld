...
class OutputChannels:
    """Singleton class to manage all OutputChannel instances"""
    
    _instance = None
    _channels: Dict[str, OutputChannel] = {}
    
    def __new__(cls):
        if cls._instance is None:
            cls._instance = super(OutputChannels, cls).__new__(cls)
            cls._instance._channels = {}
        return cls._instance
    
    @classmethod
    def get_channel(cls, task_id: str, workspace: Optional[WorkSpace] = None) -> OutputChannel:
        """
        ...
        """
        instance = cls()
        if task_id not in instance._channels:
            if not workspace:
                workspace = WorkSpace.from_local_storages(workspace_id=f"Workspace-Task_{task_id}")
            instance._channels[task_id] = OutputChannel.create(task_id=task_id, workspace=workspace)
        return instance._channels[task_id]
    
    @classmethod
    def remove_channel(cls, task_id: str) -> None:
        """
        Remove a channel by task_id
        
        Args:
            task_id: Unique identifier for the task
        """
        instance = cls()
        if task_id in instance._channels:
            del instance._channels[task_id]
    
    @classmethod
    def get_all_channels(cls) -> Dict[str, OutputChannel]:
        """Get all active channels"""
        instance = cls()
        return instance._channels.copy()
    
    @classmethod
    def clear_all_channels(cls) -> None:
        """Remove all channels"""
        instance = cls()
