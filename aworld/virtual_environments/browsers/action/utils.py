...
async def async_click_element(page: APage, element_node: DOMElementNode, **kwargs) -> Optional[str]:
    try:
        element_handle: AElementHandle = await async_get_locate_element(page, element_node)
...
def click_element(page: Page, element_node: DOMElementNode, **kwargs) -> Optional[str]:
    try:
...
async def async_get_locate_element(current_frame: APage, element: DOMElementNode) -> Optional[AElementHandle]:
...
def get_locate_element(current_frame: Page, element: DOMElementNode) -> Optional[ElementHandle]:
    # Start with the target element and collect all parents
    parents: list[DOMElementNode] = []
    current = element
    while current.parent is not None:
